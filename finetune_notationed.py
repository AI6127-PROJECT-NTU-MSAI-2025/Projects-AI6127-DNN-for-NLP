...
# from transformers.models.mbart import MBartForConditionalGeneration, MBart50TokenizerFast
...
if __name__ == "__main__":
   ...
   ########## 需要修改的参数 #########
    model_args = ModelArguments(
        model_name_or_path="models/mbart-large-cc25",  # 替换为您想使用的预训练模型路径, 如果输入huggingface模型名称下载的话，它会保存在
    ...
    training_args = Seq2SeqTrainingArguments(
        output_dir="./fine_tune_checkpoints/mbart25_finetune",  # !!! 替换为模型保存路径
        do_train=True,
        do_eval=True,
        num_train_epochs=1,
        ...
        fp16=True,  #fp16可以加速训练，但可能导致训练不稳定，use it at your own risk
        seed=42,
    ...
    if 't5' in model_args.model_name_or_path:
        ...
        model = MBartForConditionalGeneration.from_pretrained(model_args.model_name_or_path, config=config)
    elif 'mbart-large-cc25' in model_args.model_name_or_path.lower():   # 新增分支
        config = MBartConfig.from_pretrained(model_args.model_name_or_path)
        tokenizer = MBart50TokenizerFast.from_pretrained(model_args.model_name_or_path)
        model = MBartForConditionalGeneration.from_pretrained(model_args.model_name_or_path, config=config)
